...
using SymptomChecker.Extensions;
using SymptomChecker.Models;
using SymptomChecker.Services;

namespace SymptomChecker.Dialogs
{
    ...
    public class MainDialog : ComponentDialog
    {
        ...
        protected override async Task<DialogTurnResult> OnBeginDialogAsync(DialogContext innerDc, object options, CancellationToken cancellationToken = default)
        {
            ...
            if (activity.Type == ActivityTypes.Message && !string.IsNullOrEmpty(activity.Text))
            {
                ...
                var skillResult = await localizedServices.LuisServices["SymptomChecker"].RecognizeAsync<SymptomCheckerLuis>(innerDc.Context, cancellationToken);
        ...
        private async Task<DialogTurnResult> RouteStepAsync(WaterfallStepContext stepContext, CancellationToken cancellationToken)
        {
            ...
            if (activity.Type == ActivityTypes.Message && !string.IsNullOrEmpty(activity.Text))
            {
                ...
                localizedServices.LuisServices.TryGetValue("SymptomChecker", out var luisService);

                if (luisService != null)
                {
                    var result = stepContext.Context.TurnState.Get<SymptomCheckerLuis>(StateProperties.SkillLuisResult);
                    var intent = result?.TopIntent().intent;

                    switch (intent)
                    {
                        case SymptomCheckerLuis.Intent.Sample:
                            {
                                return await stepContext.BeginDialogAsync(_sampleDialog.Id, cancellationToken: cancellationToken);
                            }

                        case SymptomCheckerLuis.Intent.None:
                        default:
                            {
