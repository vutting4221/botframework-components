...
using SymptomBasedSkill.Extensions;
...
namespace SymptomBasedSkill.Dialogs
{
    ...
    public class MainDialog : ComponentDialog
    {
        ...
        private readonly PrimarySymptomsCheckDialog _primarySymptomsCheckDialog;
            ...
            : base(nameof(MainDialog))
        {
            ...
            _primarySymptomsCheckDialog = serviceProvider.GetService<PrimarySymptomsCheckDialog>();
            AddDialog(_primarySymptomsCheckDialog);
        }

        // Runs when the dialog is started.
        protected override async Task<DialogTurnResult> OnBeginDialogAsync(DialogContext innerDc, object options, CancellationToken cancellationToken = default)
        {
            //var proactiveState = await _proactiveStateAccessor.GetAsync(innerDc.Context, () => new ProactiveModel());
            //var conversationReference = proactiveState["29:1IWq2aXvmoP7ClihX7mnyqYIsmFm_zQI9WelfQwYxV7UC2_RNVjruRj8EaffaFZzsHPS_JPgHMn1slbPyx-DJRQ"].Conversation;
            //await _proactiveStateAccessor.SetAsync(innerDc.Context, proactiveState);
            ...
            if (activity.Type == ActivityTypes.Message && !string.IsNullOrEmpty(activity.Text))
            {
                ...
                var skillResult = await localizedServices.LuisServices["SymptomBasedSkill"].RecognizeAsync<SymptomBasedSkillLuis>(innerDc.Context, cancellationToken);
        ...
        private async Task<DialogTurnResult> RouteStepAsync(WaterfallStepContext stepContext, CancellationToken cancellationToken)
        {
            ...
            if (activity.Type == ActivityTypes.Message && !string.IsNullOrEmpty(activity.Text))
            {
                ...
                localizedServices.LuisServices.TryGetValue("SymptomBasedSkill", out var luisService);

                if (luisService != null)
                {
                    var result = stepContext.Context.TurnState.Get<SymptomBasedSkillLuis>(StateProperties.SkillLuisResult);
                    var intent = result?.TopIntent().intent;

                    switch (intent)
                    {
                        case SymptomBasedSkillLuis.Intent.Sample:
                            {
                                return await stepContext.BeginDialogAsync(_sampleDialog.Id, cancellationToken: cancellationToken);
                            }

                        case SymptomBasedSkillLuis.Intent.PrimaryCheck:
                            {
                                var conversationReference = stepContext.Context.Activity.GetConversationReference();
                                return await stepContext.BeginDialogAsync(_primarySymptomsCheckDialog.Id, cancellationToken: cancellationToken);
                            }

                        case SymptomBasedSkillLuis.Intent.None:
                        default:
                            {
