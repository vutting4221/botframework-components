...
namespace Microsoft.Bot.Solutions.Extensions.Input
{
    ...
    /// </summary>
    public class CustomAuthInput : InputDialog
    {
        [JsonProperty("$kind")]
        public const string DeclarativeType = "Microsoft.Bot.Solutions.CustomAuthInput";
        ...
        [JsonProperty("appId")]
        public StringExpression AppId { get; set; }

        [JsonProperty("appPassword")]
        public StringExpression AppPassword { get; set; }
        ...
        public override async Task<DialogTurnResult> BeginDialogAsync(DialogContext dc, object options = null, CancellationToken cancellationToken = default(CancellationToken))
        {
            ...
            if (this.AppId != null && this.AppPassword != null)
            {
                var appId = this.AppId.GetValue(dc.State);
                var appPassword = this.AppPassword.GetValue(dc.State);
                _appCredentials = new MicrosoftAppCredentials(appId, appPassword);
            }
            ...
            var output = await adapter.GetUserTokenAsync(dc.Context, _appCredentials, ConnectionName.GetValue(dc.State), null, cancellationToken).ConfigureAwait(false);
        ...
        public async Task<TokenResponse> GetUserTokenAsync(DialogContext dc, CancellationToken cancellationToken = default(CancellationToken))
        {
            ...
            return await adapter.GetUserTokenAsync(dc.Context, _appCredentials, ConnectionName.GetValue(dc.State), null, cancellationToken).ConfigureAwait(false);
        }
        ...
        public async Task SignOutUserAsync(DialogContext dc, CancellationToken cancellationToken = default(CancellationToken))
        {
            ...
            // Sign out user
            await adapter.SignOutUserAsync(dc.Context, _appCredentials, ConnectionName.GetValue(dc.State), dc.Context.Activity?.From?.Id, cancellationToken).ConfigureAwait(false);
        }
        ...
        private async Task SendOAuthCardAsync(DialogContext dc, IMessageActivity prompt, CancellationToken cancellationToken = default(CancellationToken))
        {
            ...
            if (!ChannelSupportsOAuthCard(turnContext.Activity.ChannelId))
            {
                ...
                {
                    var signInResource = await adapter.GetSignInResourceAsync(turnContext, _appCredentials, ConnectionName?.GetValue(dc.State), turnContext.Activity.From.Id, null, cancellationToken).ConfigureAwait(false);
        ...
        private async Task<PromptRecognizerResult<TokenResponse>> RecognizeTokenAsync(DialogContext dc, CancellationToken cancellationToken = default(CancellationToken))
        {
            ...
            else if (IsTeamsVerificationInvoke(turnContext))
            {
                ...
                try
                {
                    var token = await adapter.GetUserTokenAsync(turnContext, _appCredentials, ConnectionName.GetValue(dc.State), magicCode, cancellationToken).ConfigureAwait(false);
            ...
            else if (turnContext.Activity.Type == ActivityTypes.Message)
            {
                ...
                if (matched.Success)
                {
                    ...
                    var token = await adapter.GetUserTokenAsync(turnContext, _appCredentials, ConnectionName.GetValue(dc.State), matched.Value, cancellationToken).ConfigureAwait(false);
                    if (token != null)
                    {
                        result.Succeeded = true;
                        result.Value = token;
